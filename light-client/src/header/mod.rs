...
use parlia_ibc_proto::ibc::lightclients::parlia::v1::{Fraction, Header as RawHeader};
...
impl Header {
    ...
    pub fn verify(&self, chain_id: &ChainId, trust_level: &Fraction) -> Result<(), Error> {
        self.headers.verify(
            chain_id,
...
mod test {
    ...
    fn test_success_verify_after_checkpoint() {
        let header = create_after_checkpoint_headers();
        let mainnet = &mainnet();
        let result = header.verify(mainnet, &half());
    ...
    fn test_try_from_any() {
        ...
        header.verify(localnet, &half()).unwrap();
