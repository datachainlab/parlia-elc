...
mod test {
    ...
    fn test_success_update_state_non_epoch(#[case] hp: Box<dyn Network>) {
        ...
        let new_current_validators_hash = input.trusted_current_validators_hash.clone();
        let new_previous_validators_hash = input.trusted_previous_validators_hash.clone();
        do_test_success_update_state(
            input.header,
            input.trusted_height,
            input.trusted_current_validators_hash,
    ...
    #[test]
    fn test_error_update_state_non_neighboring_epoch() {
        ...
        let _new_current_validator_hash =
            hex!("8a4bca6491ec89e201ea0e2776f0ae42f7e5c8d67a305a7aa6284a0e0150f736");
        let _new_previous_validator_hash = trusted_previous_validator_hash;
        ...
        mock_consensus_state.insert(Height::new(0, trusted_height), trusted_cs);
        let cs = ClientState {
            chain_id: ChainId::new(56),
            ibc_store_address: hex!("151f3951FA218cac426edFe078fA9e5C6dceA500"),
        ...
        let err = client.update_client(&ctx, client_id, any).unwrap_err();
        assert!(
            format!("{:?}", err).contains("UnexpectedTrustedHeight: 39980600 39981000"),
            "{}",
            err
        );
    }
